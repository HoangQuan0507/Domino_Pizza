...
if (!isset($_GET["id"])) {
    ...
    $id = $_GET["id"];
    $errors = array();
    $old = show_old_data_code ($conn,$id);
    if (isset($_POST["create"])) {
        ...
        if (empty($_POST["saleoff"])) {
            $errors[] = "please enter your saleoff.";
        }
        if (!check_name_code_exists_for_edit ($conn,$_POST["name"],$id)) {
            $errors[] = "name code exists";
         }
         if (!check_code_exists_for_edit ($conn,$_POST["code"],$id)) {
            $errors[] = "code exists";
         }
